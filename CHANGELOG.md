...
## 5.6.0-pos — 2026-07-12

**Transparent Codex review wrapper (V2).** External Codex review was unobservable and tracked the wrong process. The pattern `timeout 900 codex exec ... 2>&1 | tail -30` hid the entire event stream until the pipeline finished, and the PID was then recovered with `pgrep -f "codex exec.*"` — which could match the parent shell, a concurrent review, or the search command itself. On 2026-06-16 it matched the parent shell and produced `tail --pid=<parent-shell> -f /dev/null`: a circular wait (shell waits on tail, tail waits on shell) that hung for **26 days**. The system was reporting "the process exists" as if it meant "Codex is working". Codex CLI was never at fault — it has supported `--json` and `--output-last-message` all along.

- **New `tools/codex-review.sh`** — the only sanctioned way to invoke external `codex exec`:
  - Unique run dir `.superflow/reviews/<slug>/<run-id>/` with `events.jsonl`, `final.md`, `stderr.log`, `status.json`, `pid`, `prompt.md`, `verdict.json`. Concurrent reviews cannot collide.
  - Launches the child as a job-controlled background job (`set -m`), so it leads its **own process group** and `$!` *is* the PGID by construction. **No `pgrep`, no `tail --pid`, no `| tail -N` anywhere in the review flow** (statically asserted by a test).
  - State machine driven by the *real* codex-cli 0.144.1 event grammar, verified live: `thread.started` → SESSION_CREATED, `turn.started` → MODEL_WAIT, `item.*` (command_execution / mcp_tool_call / file_change / reasoning / …) → TOOL_ACTIVE, `agent_message` → SYNTHESIZING, `turn.completed` → COMPLETED, plus SILENT / STALLED_SUSPECTED / TIMED_OUT / FAILED / VERDICT_PARSED. An `item.type=error` is **diagnostic only** — the real CLI emits one (config-deprecation notice) on fully successful exit-0 runs, so it never fails a review on its own.
  - `progress_confirmed` flips true only when the model actually emits items. A live process that sent the request and heard nothing back stays `progress_confirmed=false` — **liveness is never sold as progress**.
  - Transition lines on every state change + periodic heartbeat (elapsed / last-event age / remaining deadline / progress-confirmed). Short-lived states are visible, not just heartbeat samples.
  - Hard deadline enforced in-loop (NOT via an outer `timeout`, which would re-create the PID confusion): TERM → grace → KILL of the **whole process group**, so no orphaned grandchildren.
  - Prompt passed via file/stdin — never argv (stays out of the process list, no argv limit).
  - Mechanical verdict: the **last** fenced JSON block in `final.md` is parsed and schema-validated into `verdict.json`. Prose is never interpreted — a final message that merely says "I approve this" yields no verdict, exits 1, and leaves the gate CLOSED. `.par-evidence.json` is assembled only from a validated `verdict.json`.
  - `reconcile` (crash recovery) and `cleanup` subcommands verify process identity by the `(pid, starttime)` tuple, so a **reused PID is never killed**.
  - Exit codes: `0` pass-class verdict · `3` fail-class verdict · `1` no valid verdict (FAILED/TIMED_OUT/malformed → gate CLOSED) · `2` usage error. Verdict strings unchanged (Rule 3.9).
  - Feature flag `CODEX_REVIEW_WRAPPER_V2=1` (default ON). Rollback to `0` drops the state machine/heartbeat but still keeps the exact `$!`, hard deadline with process-group kill, separate stdout/stderr, and recorded exit code — it never reintroduces `pgrep`/`tail --pid`, and preserves run dirs for postmortem.
- **New `tools/test-codex-review.sh` + `tools/test-fixtures/fake-codex.sh`** — 9 hermetic tests (74 assertions): normal, silent, timeout, malformed verdict, concurrent, no-wrong-PID, early CLI failure, tool activity, crash/recovery. The fake stub replays the real JSONL grammar; no network, no real `codex`. The no-wrong-PID test spawns a decoy whose command line *would* be matched by the old `pgrep -f "codex exec"` and asserts the wrapper never waits on or kills it.
- **`--mode review` does NOT use `codex exec review`.** The first LIVE run surfaced a real CLI constraint: codex-cli 0.144.1 rejects `exec review --base <BRANCH>` together with a `[PROMPT]` positional (`-` counts as one) — `error: the argument '--base <BRANCH>' cannot be used with '[PROMPT]'`. Superflow's gate *requires* the prompt (it carries the verdict contract + criteria rubric), so review mode runs a plain `codex exec` and scopes via the prompt (`git diff <base>...HEAD`). The previously documented command in `review-unified.md` (`codex exec review --base main … - < prompt`) was **already broken** against this CLI. `fake-codex.sh` now reproduces the constraint so the suite catches it.
- **Codex technical review (round 1) → REQUEST_CHANGES, 3 HIGH findings, all confirmed by test and fixed:**
  1. *Orphaned TERM-resistant descendants.* `kill_group` waited on the **leader** only; a descendant that ignores SIGTERM survived once the leader exited. Escalation is now driven by **group** liveness (`ps -eo pgid=,stat=`, zombies excluded — a zombie is still signalable, so `kill -0 -PGID` would have reported a fully-reaped group as alive), then SIGKILL on the group, then a verified re-check.
  2. *False PASS from an earlier fence.* The extractor scanned backwards for "the last block that happens to parse", so a valid verdict followed by a truncated/garbled fence silently passed as APPROVE. The **last** fenced block must now BE the verdict — a bad last block fails closed.
  3. *False PASS from an incomplete verdict.* `findings`/`summary` were defaulted, so `{"verdict":"APPROVE"}` alone passed. All three contract fields are now mandatory.
  Round 2 re-review: see `.par-evidence`/run dir. Tests grew 74 → 82 assertions (T3 gains a TERM-resistant descendant; T4 gains the trailing-bad-fence and incomplete-verdict cases).
- **New `references/codex-review-wrapper.md`** — wrapper reference (run-dir layout, states, event grammar, verdict contract, flag/rollback).
- **Wiring:** `superflow-enforcement.md` (Rule 3.2/3.2a technical reviewer + Rule 3.3 verdict source + Secondary Provider Invocation, now with an explicit FORBIDDEN block), `references/phase2/steps/review-unified.md` (technical-lens primary), `references/phase2/steps/holistic-review.md`, `references/phase2/steps/par-evidence.md` (verdict read from validated `verdict.json`; no `verdict.json` = not a pass), `CLAUDE.md`, `llms.txt`.
