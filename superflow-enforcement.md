...
13. **(removed in v5.4.0)** — the event-telemetry stack (the emit helper, its schema file, and every emit call) was deleted after zero real usage; `.superflow-state.json` is the single source of truth. Number retained so cross-references to Rule 14 stay stable.
