...
| `tools/test-codex-review.sh` | 19 hermetic test groups (185 assertions) for the wrapper (normal, silent, timeout, malformed verdict, concurrent, no-wrong-PID, early failure, tool activity, crash/recovery) using the `tools/test-fixtures/fake-codex.sh` stub — no network, no real `codex` |
