...
| `tools/test-codex-review.sh` | 9 hermetic tests for the wrapper (normal, silent, timeout, malformed verdict, concurrent, no-wrong-PID, early failure, tool activity, crash/recovery) using the `tools/test-fixtures/fake-codex.sh` stub — no network, no real `codex` |
