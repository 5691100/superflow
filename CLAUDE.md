...
| `tools/codex-review.sh` | **The only sanctioned way to invoke external Codex.** Transparent review wrapper (V2): unique run dir, `codex exec --json` event stream, state machine + heartbeat, exact `$!`/PGID (no `pgrep`), hard deadline with process-group kill, mechanical fenced-JSON verdict validation. Subcommands `run` / `reconcile` / `cleanup`. Exit 0=pass-class, 3=fail-class, 1=no valid verdict (gate CLOSED), 2=usage |
| `tools/test-codex-review.sh` | 20 hermetic test groups (197 assertions) for the wrapper (normal, silent, timeout, malformed verdict, concurrent, no-wrong-PID, early failure, tool activity, crash/recovery) using the `tools/test-fixtures/fake-codex.sh` stub — no network, no real `codex` |
| `references/codex-review-wrapper.md` | Wrapper reference — run-dir layout, state machine, real codex-cli 0.144.1 event grammar, verdict contract, feature flag + rollback |
...
- **External Codex invocation (V2 wrapper)**: all `codex exec` calls go through `tools/codex-review.sh` (`CODEX_REVIEW_WRAPPER_V2=1`, default ON). It captures the exact `$!` at launch and puts the child in its own process group — `pgrep`, `tail --pid`, and `codex exec … | tail -N` are FORBIDDEN (they hid the event stream and, on 2026-06-16, produced a 26-day circular wait on the wrong PID). The wrapper owns its hard deadline, so never nest it inside another `timeout`. It validates the fenced-JSON verdict into `verdict.json`; a missing/malformed verdict is never a pass. Reference: `references/codex-review-wrapper.md`.
