...
13. **(removed in v5.4.0)** — the event-telemetry stack (the emit helper and every emit call) was deleted; `.superflow-state.json` is the single source of truth. Number retained so cross-references to Rules 14/15 stay stable.
