...
At Phase 0 start, write `.superflow-state.json`. **MERGE into any existing state — never overwrite the file wholesale.** SKILL.md startup writes `context.run_id` before Phase 0 begins; clobbering it drops the run's state-recovery key:
...
If python3 is unavailable, rewrite the file manually — but first read the existing JSON and carry over every key you are not changing, especially `context` (it holds `context.run_id`).
