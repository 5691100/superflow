...
- **args:** `{sprint, branch, base, charter_path, workdir, spec_path, plan_path, brief_path, product, diff_hint}`
  - `workdir` (required): absolute path to the sprint worktree (or repo root for
    `solo_single_pr`). The technical reviewer cds into workdir before running
    `tools/codex-review.sh`; the product reviewer uses `git -C workdir diff` — without this,
    both would diff the wrong checkout.
...
- Runs in parallel: a product reviewer (follows `~/.claude/agents/standard-product-reviewer.md`,
  ...
  the Codex CLI exists, `bash tools/codex-review.sh run --slug sprint-<N>-technical --base <base>
  --effort high --prompt-file <prompt>` — the transparent wrapper, never raw `codex exec` and never
  inside an outer `timeout`; read the verdict from `<run-dir>/verdict.json`;
