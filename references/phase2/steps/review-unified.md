...
**Contract criteria (Rule 3a).** Unified Review verifies the sprint contract
(`docs/superflow/contracts/<date>-<feature>.contract.yml`) criterion-by-criterion — not the raw
request. Each reviewer's dispatch prompt includes the contract, and the orchestrator records a
per-criterion PASS/FAIL table in `.par-evidence.json` under `criteria` (e.g. `{"C01":"PASS",...}`)
in addition to the overall verdict. See `references/contract-gate.md`. UI sprints
(`review_required.browser`) add a black-box browser pass; artifact sprints
(`review_required.artifact`) open & reconcile every produced file against golden fixtures.
