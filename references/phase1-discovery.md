...
2. **Secondary provider (TECHNICAL lens)**: write the spec + instructions to a prompt file, then `bash tools/codex-review.sh run --mode exec --slug spec-review --effort high --prompt-file /tmp/spec-review.md` via Bash (`run_in_background: true`). Transparent wrapper only — never raw `codex exec`, never `| tail -N`, never an outer `timeout`. Verdict: `jq -r .verdict "$(ls -d .superflow/reviews/spec-review/* | tail -1)/verdict.json"`.
...
2. **Secondary provider (TECHNICAL lens)**: write the plan + instructions to a prompt file, then `bash tools/codex-review.sh run --mode exec --slug plan-review --effort high --prompt-file /tmp/plan-review.md` via Bash (`run_in_background: true`). Transparent wrapper only — never raw `codex exec`, never `| tail -N`, never an outer `timeout`. Are there missing tasks? Over-engineering? Does sprint ordering make sense? Verdict: `jq -r .verdict "$(ls -d .superflow/reviews/plan-review/* | tail -1)/verdict.json"`.
