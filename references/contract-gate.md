...
3. **Evaluator = Codex** (the technical reviewer; Claude technical reviewer only as fallback when Codex is unavailable). Run `codex exec` on the contract file. Codex reviews it for *sufficiency*: missing scenarios, edge cases, empty/error states, regression risk, owner directives, atomicity/concurrency. It replies with additions — not code feedback. The orchestrator blocks the sprint until Codex confirms.
