...
3. **Evaluator = Codex** (the technical reviewer; Claude technical reviewer only as fallback when Codex is unavailable). Run the contract file through the transparent wrapper — `bash tools/codex-review.sh run --mode exec --slug contract-gate --effort high --prompt-file docs/superflow/contracts/<date>-<feature>.contract.yml` — never raw `codex exec`, never `| tail -N`, never an outer `timeout` (see `references/codex-review-wrapper.md`). Codex reviews it for *sufficiency*: missing scenarios, edge cases, empty/error states, regression risk, owner directives, atomicity/concurrency. It replies with additions — not code feedback. The orchestrator blocks the sprint until Codex confirms.
